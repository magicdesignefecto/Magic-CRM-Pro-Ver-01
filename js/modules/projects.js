            ...
            ${Modal.render('Nuevo Proyecto', `
                <form id="projectForm">
                    ...
                    <div class="form-section" style="background: rgba(16,185,129,0.1);">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                <label class="form-label">Monto recibido (Bs.) *</label>
            ...
            <div class="project-card status-${statusClass}">
                ...
                <div class="project-finance">
                    <div>
                        ...
                        <div class="finance-value">Bs. ${budget.toLocaleString('es-ES', { minimumFractionDigits: 2 })}</div>
                    </div>
                    ...
                    <div style="text-align: right;">
                        ...
                        <div class="finance-value ${p.tipoCambio > 0 ? 'positive' : ((budget - (p.costs || 0)) >= 0 ? 'positive' : 'negative')}">${p.tipoCambio > 0 ? '$' + usd.toFixed(2) : 'Bs. ' + (budget - (p.costs || 0)).toFixed(2)}</div>
                    </div>
                </div>
...
<head>
    ...
    <style>
        *, html, body { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Footer */
        .report-footer { margin-top: 20px; padding: 20px 24px; background: #F8FAFC; border-top: 1px solid #E2E8F0; text-align: center; }
        .footer-left { font-size: 0.65rem; color: #94A3B8; }
        .footer-right { font-size: 0.65rem; color: #94A3B8; margin-top: 4px; }
...
<body>
    <div class="page">
        ...
        <div class="content">
            ...
            ${(p.payments || []).length > 0 ? (() => {
                ...
                const totalGastos = (p.payments || []).filter(x => x.type === 'expense').reduce((s, x) => s + x.amount, 0);
                const totalCobros = (p.payments || []).filter(x => x.type === 'income').reduce((s, x) => s + x.amount, 0);
                const presupuesto = hasConversion ? usd : budget;
                const sobrante = presupuesto - totalGastos + totalCobros;
                return `
            <div class="section">
        ...
        <div class="report-footer">
            <div class="footer-left">Magic Design Efecto · CRM · Generado: ${new Date().toLocaleString('es-ES', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>
            <div class="footer-right">Responsable: ${responsable} · Documento generado automáticamente</div>
        </div>
    </div>
    ...
    <script>
        document.getElementById('btnDownload').addEventListener('click', function() {
            ...
            // Convert exact pixel dimensions to mm (1px = 0.2646mm at 96dpi)
            // This creates a 1:1 copy — no text reflow, no page breaks
            const pxToMm = 0.2646;
